...
const setupInAnimation = (about: HTMLElement) => {
  inMM = createMatchMedia((_context, { isMobile, isLandscape }) => {
    ...
    tl.fromTo(
      sceneWeightsInOut.hero,
      { out: 0 },
      { out: 1, ease: "none", duration: 1 },
      0,
    );
    ...
    tl.fromTo(
      ...
      { x: 0, y: 0, z: 0 },
      { y: -1.1, duration: 0.6, ease: "power4.out" },
      0,
    );
    tl.fromTo(
      room.chairScrollRotation,
      { x: 0, y: 0, z: 0 },
      { x: -0.9, z: -1.3, duration: 0.6 },
      0,
    );
    ...
    tl.fromTo(
      ...
      { value: 0 },
      { value: 1, duration: 0.75, ease: "power1.out" },
      0,
    );

    tl.fromTo(
      sceneWeightsInOut.about,
      { in: 0 },
      { in: 1, ease: "none", duration: 1 },
      0,
    );
    tl.fromTo(
      sceneWeightsInOut["about-1"],
      { in: 0 },
      { in: 1, ease: "none", duration: 1 },
      0,
    );
    ...
    tl.fromTo(
      room.group.scale,
      { x: 1, y: 1, z: 1 },
      { x: 0.85, y: 0.85, z: 0.85, duration: 1, ease: "none" },
      0,
    );
    if (!isLandscape) {
      tl.fromTo(
        room.group.position,
        { x: 0, y: 0, z: 0 },
        { x: 0, y: 5.4, z: 0, duration: 1, ease: "none" },
        0,
      );
      tl.fromTo(
        room.group.rotation,
        { x: 0, y: -2.1, z: 0 },
        { x: 0, y: -2.1, z: 0, duration: 1, ease: "none" },
        0,
      );
    } else {
      tl.fromTo(
        room.group.position,
        { x: 2, y: 0, z: 0 },
        { x: 4.5, y: 5.7, z: 0, duration: 1, ease: "none" },
        0,
      );
      tl.fromTo(
        room.group.rotation,
    ...
    if (isLandscape) {
      ...
      tl.fromTo(
        lab.group.position,
        { x: 0, y: 0, z: 6 },
        { x: 0, y: 0, z: 6, duration: 1, ease: "power.1out" },
        0,
      );
      ...
      tl.fromTo(
        waypointsPosition,
        { x: 2, y: 0, z: 0 },
        { x: 0, y: 0, z: 6, duration: 1, ease: "power1.out" },
        0,
      );
      tl.fromTo(
        waypointsRotation,
      ...
      tl.fromTo(
        lab.group.position,
        { x: 0, y: 0, z: 6 },
        { x: 0, y: 0, z: 6, duration: 1, ease: "none" },
        0,
      );
...
const setupScenesAnimation = (about: HTMLElement) => {
  scenesMm = createMatchMedia(() => {
    ...
    tl.to(
      waypointsRotation,
      { x: 0, y: -Math.PI, z: 0, duration: duration, ease: "power1.inOut" },
      delay,
    );

    tl.to(
      sceneWeightsInOut["about-2"],
      { in: 1, duration: duration, ease: "power1.inOut" },
      delay,
    );
    tl.to(
      sceneWeightsInOut["about-1"],
      { out: 1, duration: duration, ease: "power1.inOut" },
      delay,
    );
  });
};
...
const setupSectionsAnimation = ({
  ...
  sectionsMm = createMatchMedia((_context, { isLandscape }) => {
    ...
    if (isLandscape) {
      ...
      tl.to(
        contentDescription,
        { opacity: 0, y: "-10vh", duration: 0.15, ease: "power1.out" },
