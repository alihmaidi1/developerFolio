...
import { stopSnoreRepetition } from "../../../sounds/core/contact";
...
const getActionFromMesh = (name: string) => {
  ...
  const action = resource.animations.find(
    (animation: AnimationClip) => animation.name === name,
  );
...
const setupHologramActions = () => {
  ...
  //left-desktop
  const leftDesktop = hologramMixer.clipAction(
    getActionFromMesh("left-desktop"),
  );
  leftDesktop.repetitions = 1;
  leftDesktop.clampWhenFinished = true;
...
const play = (name: string, transition: number = 0.5) => {
  ...
  if (!newAction || !newHologramAction)
    throw new Error("[AvatarAnimations] Action not found");
...
const updateIntro = () => {
  setWeight(
    "desktop-idle",
    (1 - avatar.tIdleIntensity.value) * (1 - wavingStrength.value),
  );
  setWeight(
    "left-desktop",
    (1 - avatar.tIdleIntensity.value) * (1 - wavingStrength.value),
