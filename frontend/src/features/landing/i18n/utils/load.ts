...
const pickModule = (ns: MessagesNamespace, locale: Locale) =>
  namespaces[ns][`./namespaces/${ns}/${locale}.json`];
  ...
  try {
    ...
    const p = (async () => {
      try {
        ...
        const data: Messages =
          (mod as { default?: Messages }).default ?? (mod as Messages);
    ...
    console.error(
      `Error loading translations for namespace ${ns} and locale ${locale}:`,
      error,
    );
    return null;
  }
