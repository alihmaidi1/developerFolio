...
function weightedAverage<T extends { x: number; y: number; z: number }>(
  points: T[],
  weights: number[],
): T {
  let total = 0,
...
let weights: number[] = [];
let resolvedPoints: typeof points.landscape | typeof points.portrait =
  points.landscape;
...
function updateReferences() {
  ...
  const active = Object.entries(sceneWeights).filter(
    ([key, weight]) => weight > 0 && key in resolvedPoints,
  ) as [SceneKey, number][];

  positions = active.map(
    ([key]) => resolvedPoints[key as keyof typeof resolvedPoints]!.position,
  );
  focuses = active.map(
    ([key]) => resolvedPoints[key as keyof typeof resolvedPoints]!.focus,
  );
